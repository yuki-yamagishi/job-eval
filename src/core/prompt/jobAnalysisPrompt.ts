...
1. 健康保険組合 (Health Insurance):
   - 加入している健康保険組合の正確な正式名称（例: 「サイバーエージェント健康保険組合」「日立健康保険組合」「関東ITソフトウェア健康保険組合 (ITS健保)」「東京都情報サービス産業健康保険組合 (TJK)」「全国健康保険協会 (協会けんぽ)」など）
   - 「自社健保」などの抽象語ではなく、会社名やグループ名を含む実在する健康保険組合の正式名称（実名）を可能な限り特定して出力してください。
   - その健保に加入していることによる具体的なメリット（保険料率の安さ/手取り有利、高額療養費付加給付、直営保養所・提携レストラン、人間ドック補助など）
   - 確度: high (採用情報・公式リリース・健保組合名簿等に明記), medium (企業規模やグループ構成から高確率), low (推定)
...
以下のキー構造を持つ純粋なJSONオブジェクトのみを出力してください（Markdownコードブロックは不要、またはJSONのみを含めること）：
{
  ...
  "health_insurance": {
    "name": "健康保険組合の正式名称（例: サイバーエージェント健康保険組合、関東ITソフトウェア健康保険組合 等）",
    "type": "corporate",
    "confidence": "high",
  ...
  "summary_advice": "手取りや福利厚生面での総合アドバイス"
}`;
...
Google検索ツールで「${companyName} 健康保険組合」および「${companyName} 確定拠出年金 福利厚生」を調査し、実在する健康保険組合の正式名称と判明した事実をJSON形式で出力してください。`;
